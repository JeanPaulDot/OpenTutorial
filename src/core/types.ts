...
export type TourTrigger =
  | ({ type: 'manual' } & TriggerBase)
  | ({ type: 'auto' } & TriggerBase)
  | ({ type: 'event'; event: string } & TriggerBase)
  /** Fires when the current path matches. `path` supports a trailing `*` wildcard. */
  | ({ type: 'route'; path: string; exact?: boolean } & TriggerBase)
  /** Fires when an element matching `selector` enters the DOM. */
  | ({ type: 'element'; selector: string; timeout?: number } & TriggerBase)
  /** Fires after `ms` without pointer/key input. */
  | ({ type: 'idle'; ms: number } & TriggerBase)
  /** Fires once the page has been scrolled `percent` (0-100) of the way down. */
  | ({ type: 'scroll'; percent: number } & TriggerBase);
...
export type I18nContent = string | { key: string; fallback?: string };
...
export type ContentBlock =
  ...
  | { type: 'list'; items: I18nContent[]; ordered?: boolean }
  | { type: 'code'; value: string; lang?: string }
  | { type: 'divider' }
  /** Raw HTML. Ignored unless the host opts in with `allowHtml: true`. */
  | { type: 'html'; value: string };

export type StepContent = I18nContent | { blocks: ContentBlock[] };
...
export type ValidationResult =
  | { ok: true; errors: never[]; warnings: SpecIssue[] }
